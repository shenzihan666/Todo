...
Deep Agents come with a local filesystem to offload memory. By default, this filesystem is stored in agent state and is __transient to a single thread__. You can extend Deep Agents with __long-term memory__ by using a `CompositeBackend` that routes specific paths to persistent storage.
...
When using `CompositeBackend`, Deep Agents maintain __two separate filesystems__:
...
- __Development__: Use `InMemoryStore` for quick iteration
- __Production__: Use `PostgresStore` or other persistent stores
- __Multi-tenant__: Consider using `assistant_id`-based namespacing

---

*For full documentation, see https://docs.langchain.com/oss/python/deepagents/long-term-memory*
