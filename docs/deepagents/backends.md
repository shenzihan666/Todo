...
| --- | --- |
| Default | `agent = create_deep_agent()` Ephemeral in state. The default filesystem backend for an agent is stored in `langgraph` state. Note that this filesystem only persists _for a single thread_. |
...
| Durable store (LangGraph store) | `agent = create_deep_agent(backend=lambda rt: StoreBackend(rt))` This gives the agent access to long-term storage that is _persisted across threads_. This is great for storing longer term memories or instructions that are applicable to the agent over multiple executions. |
